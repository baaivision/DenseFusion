...
from densefusion import conversation as conversation_lib
from densefusion.constants import (DEFAULT_IM_END_TOKEN, DEFAULT_IM_START_TOKEN,
                               DEFAULT_IMAGE_TOKEN, IGNORE_INDEX)
from densefusion.mm_utils import tokenizer_image_token
from densefusion.model import *
from densefusion.train.llava_trainer import LLaVATrainer
...
def preprocess_plain_text(
    ...
    for source in sources:
        assert len(source) == 2
        has_image = (DEFAULT_IMAGE_TOKEN in source[0]['value'])
        ...
        if has_image:
            ...
            conversation = source[1]['value'] + conversation_lib.default_conversation.sep
            conversations.append(conversation)
    input_ids = tokenizer(
        conversations,
        return_tensors="pt",
